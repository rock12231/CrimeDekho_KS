...
class CrimeAnalysisTable(View):
    ...
    def get(self, request):
        if request.user.is_authenticated:
            ...
            # rjdf = pd.DataFrame(FirData.objects.all().values())
            rjdf = pd.DataFrame(Fir_Karnataka.objects.all().values()[0:10000])
            # Paginate the data
            paginator = Paginator(rjdf, 10000)  # Show 10 records per page
...
#             data_dict = rjdf.to_dict('records')
#             JSONdata = json.dumps(data_dict, ensure_ascii=False)
#             return render(request, 'CrimeAnalysis/table.html', {'JSONdata':JSONdata})
